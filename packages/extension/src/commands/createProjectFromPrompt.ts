...
export async function createProjectFromPrompt(): Promise<void> {
  // Get available templates
  let templates: any[];
  try {
    const templatesResponse = await callServer('/api/projects/templates', {});
  ...
  try {
    ...
    const result = await callServer('/api/projects/from-prompt', {
      prompt: prompt || '',
    ...
    if (result.success) {
      vscode.window.showInformationMessage(
        `✅ Project created! ${result.filesCreated.length} files created, ${result.filesModified.length} files customized by AI.`
      );
