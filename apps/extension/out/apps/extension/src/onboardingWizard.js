...
class OnboardingWizard {
    ...
    async show() {
        ...
        if (onboardingCompleted) {
            const restart = await vscode.window.showInformationMessage('KRINS Code Coordination is already set up. Would you like to run the setup wizard again?', 'Yes, restart setup', 'No');
    ...
    async selectRepository() {
        try {
            ...
            if (fs.existsSync(packageJsonPath)) {
                ...
                if (deps.react) {
                    if (deps.next)
                        repoInfo.projectType = 'Next.js';
                    else if (deps.gatsby)
                        repoInfo.projectType = 'Gatsby';
                    else if (deps['react-native'])
                        repoInfo.projectType = 'React Native';
                    else
                        repoInfo.projectType = 'React';
                }
                // Vue ecosystem
                else if (deps.vue) {
                    if (deps.nuxt)
                        repoInfo.projectType = 'Nuxt.js';
                    else
                        repoInfo.projectType = 'Vue';
                }
                // Angular
    ...
    async analyzeProject() {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder)
            return 'No workspace to analyze';
    ...
    async completeOnboarding() {
        try {
            ...
            const action = await vscode.window.showInformationMessage('🎉 Welcome to KRINS Code Coordination! You\'re all set to start multi-AI powered development.', 'Open Dashboard', 'Assign First Task', 'View Documentation');
            switch (action) {
                case 'Open Dashboard':
