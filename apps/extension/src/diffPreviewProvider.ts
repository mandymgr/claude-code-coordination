...
export class DiffPreviewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'claude-coordination-diff-preview';
  private _view?: vscode.WebviewView;
  private _socket?: Socket;
  private _pendingDiffs: DiffPreviewData[] = [];
  ...
  private initializeWebSocket() {
    try {
      ...
      this._socket.on('diff-generated', (diffData: DiffPreviewData) => {
        console.log('📝 New diff generated:', diffData);
        this._pendingDiffs.unshift(diffData);
        this.refreshWebview();
      });

      this._socket.on('diff-applied', (diffId: string) => {
  ...
  private async applyDiff(diffId: string) {
  ...
  private async rejectDiff(diffId: string) {
    const diff = this._pendingDiffs.find(d => d.id === diffId);
    if (!diff) return;
  ...
  private async previewDiff(diffId: string) {
    const diff = this._pendingDiffs.find(d => d.id === diffId);
    if (!diff) return;
  ...
  private generateDiffView(diff: DiffPreviewData): string {
...
${this.generateUnifiedDiff(diff.originalContent, diff.diffContent)}`;
  ...
  public addDiff(diffData: DiffPreviewData) {
    this._pendingDiffs.unshift(diffData);
    this.refreshWebview();
  }
