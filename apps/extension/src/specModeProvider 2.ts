...
export class SpecModeProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'claude-spec-mode';
  private _view?: vscode.WebviewView;
  private _context: vscode.ExtensionContext;
  private _currentResult?: SpecModeResult;
  ...
  public async runSpecMode(request: SpecModeRequest) {
    if (this.isRunning) {
    ...
    try {
      ...
      const response = await axios.post(`${this.serverUrl}/api/tasks/spec`, {
        task: request.task,
      ...
      if (this._currentResult.success) {
        vscode.window.showInformationMessage(
          `✅ TDD completed successfully! Generated ${this._currentResult.specGeneration.totalTests} tests`
        );
      } else {
        vscode.window.showWarningMessage(
          `⚠️ TDD completed with issues. ${this._currentResult.specGeneration.totalTests} tests generated but implementation needs work`
        );
      }
  ...
  public async generateTestsOnly(request: SpecModeRequest) {
    if (this.isRunning) {
      vscode.window.showWarningMessage('Spec Mode is already running');
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(`${this.serverUrl}/api/tasks/spec/tests-only`, {
        task: request.task,
        targetFiles: request.targetFiles,
        framework: request.framework || 'vitest'
      });
      ...
      vscode.window.showInformationMessage(
        `✅ Generated ${result.totalTests} test specifications (confidence: ${Math.round(result.confidence * 100)}%)`
      );
  ...
  public async applyGeneratedPatch() {
    ...
    const choice = await vscode.window.showInformationMessage(
      'Apply the generated patch to your files?',
      'Yes', 'Review First', 'Cancel'
    );
  ...
  private async showPatchDiff(patch: string) {
  ...
  private async openTestFile(filePath: string) {
    try {
      const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
  ...
  private updateWebview(state?: 'running' | 'error') {
