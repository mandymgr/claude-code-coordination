...
export class QualityGateProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'claude-quality-gate';
  private _view?: vscode.WebviewView;
  private _context: vscode.ExtensionContext;
  private _currentResult?: QualityGateResult;
  ...
  public async runQualityGates() {
    try {
      ...
      const response = await axios.post(`${this.serverUrl}/api/quality/validate`, {
        changedFiles,
  ...
  public async applyFix(suggestionId: string) {
    ...
    try {
      ...
      const response = await axios.post(`${this.serverUrl}/api/quality/fix`, {
  ...
  public async applyAllFixes() {
    if (!this._currentResult || this._currentResult.suggestions.length === 0) return;
    ...
    try {
      const choice = await vscode.window.showInformationMessage(
        `Apply ${automatableSuggestions.length} automated fixes?`,
        'Yes', 'No'
      );
  ...
  private async getChangedFiles(): Promise<string[]> {
    return new Promise((resolve) => {
      ...
      exec('git diff --name-only HEAD', (error: any, stdout: string) => {
        if (error) {
          resolve([]);
          return;
        }
  ...
  private updateWebview(state?: 'loading' | 'error') {
